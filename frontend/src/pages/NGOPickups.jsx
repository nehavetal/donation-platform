...
export default function NGOPickups() {
  ...
  const load = async () => {
    const res = await api.get(`/pickups/ngo/${ngo._id}`);
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>🚚 NGO Pickups Dashboard</h2>

      {pickups.length === 0 ? (
        <p style={styles.empty}>No pickups found</p>
      ) : (
        pickups.map((p) => (
          <div key={p._id} style={styles.card}>
            ...
            <div style={styles.btnGroup}>
              <button
                style={styles.btn}
                onClick={() => updateStatus(p._id, "assigned")}
              >
                Assign
              </button>
